...
# modified 05.10.2001 mst: debug=1
...
# mst: this is the path to iptables in SuSE 7.2
...
if ($DEBUG) {
  # write @e to file
  open(DEBUG, ">/etc/bitloop/fwcustomrc");
	print DEBUG "fw_custom_before_antispoofing()\n{\n";	
   print DEBUG join("\n", @e);
	print DEBUG "\n}\n";
  close(DEBUG);
} else {
  ...
  if (uc($values[39]) eq 'YES') {
    ...
    for (my $n=0; ($n < scalar(@values)) && ($n < scalar(@fields)); $n++) {
      ...
      next if (! $values[$n] || (uc($values[$n]) eq 'IGNORE') ||
               ! $fields[$n] || (uc($values[$n]) eq 'NO'));
# added last condition, 05.10.2001 mst
      if ($fields[$n] =~ /^B(.*)/) {
