...
&terror('crule_err_acl') if (! $access{'crules'});

&terror('crule_err_norule') if ($in{'rule'} eq "");
&terror('crule_err_notable') if (! $in{'table'});
&terror('crule_err_nochain') if (! $in{'chain'});
...
my @tmpl=($file->[$rule->{'line'}], $file->[$rule->{'line'}]);
splice(@$file, $rule->{'line'}, 1, @tmpl);
